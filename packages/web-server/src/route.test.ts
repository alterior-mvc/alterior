...
import { Controller, Delete, Get, Mount, Options, Patch, Post, Put, RequestBase, ResponseBase, WebEvent } from './metadata';
...
function fakeAppVarietyOfMethods() {
    @WebService({
        server: {
            ...
            silent: true,
            middleware: [
                (req: RequestBase, res: ResponseBase, next: () => void) => {
    ...
    describe("RouteDecorator", it => {
        ...
        it('should apply path-limited middleware', async () => {
            @WebService({
                server: {
                    middleware: [
                        ['/foo', (req: RequestBase & { fun?: string }, res: ResponseBase, next: () => void) => {
                            req.fun = 'funfun';
                            next();
                        }]
                    ]
        ...
        it('mounted controllers should inherit middleware from parent controller', async () => {
            ...
            let counter = 0;
            function counterMiddleware(req: RequestBase, res: ResponseBase, next: () => void) {
                ++counter;
                next();
            }
        ...
        it('should be able to inject WebEvent instead of request/response', async () => {
            ...
            expect('send' in observedEvent!.response).to.be.true;
        });
