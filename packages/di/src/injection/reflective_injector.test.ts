...
suite(describe => {
  ...
  describe('resolve', it => {
    it('should resolve and flatten', () => {
      ...
      providers.forEach(function(b) {
        ...
        expect(b instanceof ResolvedReflectiveProvider_).to.eq(true);
      });
    });
    ...
    it('should allow declaring dependencies with flat arrays', () => {
      const resolved = ReflectiveInjector.resolve([{ provide: 'token', useFactory: (e: any) => e, deps: [new InjectAnnotation('dep')] }]);
      const nestedResolved = ReflectiveInjector.resolve([{ provide: 'token', useFactory: (e: any) => e, deps: [[new InjectAnnotation('dep')]] }]);
  ...
  describe('displayName', it => {
    it('should work', () => {
      expect((<ReflectiveInjector_>ReflectiveInjector.resolveAndCreate([Engine, BrokenEngine])).displayName).to.equal(
