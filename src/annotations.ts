...
export class AngularCompatibleAnnotation extends Annotation implements IAnnotation {
    constructor(
        props? : any
    ) {
        super(props);
        this.ngMetadataName = (this.constructor as any).getNgMetadataName();
    }
    
    ngMetadataName : string;
    
    static getNgMetadataName(): string {
        if (!this['ngMetadataName'])
            throw new Error(`You must decorate your AngularCompatibleAnnotation subclass with @NgMetadataName()`);

        return this['ngMetadataName'];
    }
